...
{
	const float w = ((v.x * m._14) + (v.y * m._24) + (v.z * m._34) + (1.f * m._44));
	const float invW = 1.f / w;

	return {
		((v.x * m._11) + (v.y * m._21) + (v.z * m._31) + (1.f * m._41)) * invW,
		((v.x * m._12) + (v.y * m._22) + (v.z * m._32) + (1.f * m._42)) * invW,
		((v.x * m._13) + (v.y * m._23) + (v.z * m._33) + (1.f * m._43)) * invW
	};
}
...
Matrix4 MathHelper::Adjoint(const Matrix4& m)
{
	return
	{
		...
		-(m._11 * ((m._32 * m._44) - (m._42 * m._34)) - m._31 * ((m._12 * m._44) - (m._42 * m._14)) + m._41 * ((m._12 * m._34) - (m._32 * m._24))),
